...
dllDirectory_Path='lib\'  %at distribution in this file near the driver, otherwise change here.
%-----------------------------------------------------------------------------------------------------------------------

addpath(dllDirectory_Path) %add the path where the .dll resides

%-----------------------------------------------------------------------------------------------------------------------
QlittleRun=1;  %number of Lambert problems to solve at first, (to test keep small, but we can solve on the order of ~1e6 per second single solutions)
...
qthSol=max(floor(rand(1)*(Q+1)),1)   %pick a solution (1<=qthSol<=Q)
